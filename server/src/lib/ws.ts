        ...
        ws.on('message', (message: string) => {
            ...
            switch(event.type) {
                case 'getRouterRtpCapabilities':
                    ...
                    break;
                case 'createProducerTransportOnServer':
                    createProducerTransport(ws);
                    break;
                ...
                case 'consumeProducer':
                    ...
                    break;
                case 'pauseProducer':
                    pauseProducer(websock, event);
                    break;
                case 'resumeProducer':
                    resumeProducer(websock, event);
                    break;
                default:
                    break;
            }
...
const getRouterRtpCapabilities = (ws: WebSocket) => {
    send(ws, 'routerRtpCapabilities', mediasoupRouter.rtpCapabilities);
}

// Create producer transport
const createProducerTransport = async (ws: WebSocket) => {
    try {
...
// Pause producer
const pauseProducer = async (websock: WebSocket.Server, event: any) => {
    let host = hosts.find(host => host.userId === event.data.producerUserId);
    let producer = host.producers.find(p => p.id === event.data.producerId);
...
const resumeProducer = async (websock: WebSocket.Server, event: any) => {
    let host = hosts.find(host => host.userId === event.data.producerUserId);
    let producer = host.producers.find(p => p.id === event.data.producerId);
    for (const client of clients) {
        const consumer = client.consumers.find(c => c.producerId === producer.id);
...
const resumeConsumer = async (event: any) => {
    let client = clients.find(c => c.userId === event.data.userId);
    const consumer = client.consumers.find(consumer => consumer.id === event.data.id);
    await consumer.resume();
}
